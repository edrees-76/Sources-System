...
public class RadioisotopeServiceTests : IClassFixture<SqliteInMemoryFixture>, IDisposable
{
    ...
    #region ي. الترجمة الإنجليزية لرسائل التحقق والنجاح (Round 135)
    ...
    public void Create_Messages_UseEnglishStrings_WhenEnglishLanguageActive()
    {
        Sources.Tests.Fixtures.WpfStaFixture.RunInSta(() =>
        {
            ...
            try
            {
                ...
                var failResult = _sut.Create(new Radioisotope
                {
                    Symbol = "Co-60",
                    Name = "Cobalt-60",
                    HalfLife = 0
                ...
                Assert.Equal("Radioisotope added successfully", createResult.Message);
                Assert.Equal(TranslationHelper.GetString("MsgSuccessRadioisotopeCreated"), createResult.Message);
                Assert.NotEqual("تم إضافة النظير بنجاح", createResult.Message);
            }
            finally
            {
