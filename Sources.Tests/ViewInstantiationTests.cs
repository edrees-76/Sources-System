...
public class ViewInstantiationTests
{
    ...
    [Fact]
    public void SourcesView_ByDefault_NeutronTypesOverlayHostIsCollapsed()
    {
        RunInSta(() =>
        {
            var mockSourceService = new Moq.Mock<Sources.Services.ISourceService>();
            ...
            // Regression guard for round 140: overlay management state must be off by default.
            Assert.Null(vm.NeutronTypesManagementViewModel);
            ...
            // A binding that relies on inherited DataContext only attaches once the element is
            // connected to a real PresentationSource (i.e. hosted in a Window). Without this,
            // Measure/Arrange alone leave the binding "Unattached" and Visibility silently reads
            // back its CLR default (Visible) regardless of the fix, making the assertion below
            // meaningless. Hosting in a real (offscreen) Window is required to exercise the actual
            // production binding-resolution path this regression test guards against.
            ...
            try
            {
                ...
                var overlay = FindVisualChildren<Sources.Views.NeutronSourceTypesOverlay>(view).FirstOrDefault();
                Assert.NotNull(overlay);

                var host = System.Windows.Media.VisualTreeHelper.GetParent(overlay) as System.Windows.Controls.Grid;
                Assert.NotNull(host);

                // Pre-fix, this Visibility bound against the overlay's own DataContext (NeutronSourceTypesViewModel),
                // which has no IsManagingNeutronTypes property, so the binding failed silently and the property
                // fell back to its default value: Visible. The overlay then covered the entire Sources section
                // regardless of state. Post-fix, Visibility is bound on the wrapping Grid, which inherits
                // SourcesViewModel as its DataContext, so it correctly resolves to Collapsed when not managing types.
                Assert.Equal(System.Windows.Visibility.Collapsed, host!.Visibility);
            }
            finally
            {
                window.Close();
