...
* آخر عمل منجز:
  - توحيد تنسيق تلميحات الرسوم البيانية (Tooltips) في منحنى التحلل بحاسبة النشاط ولوحة التحكم (commit 6173f51).
  - إضافة مشروع اختبارات وحدة منفصل (Sources.Tests) لخدمة DecayCalculationService — 99 اختبار، نسبة نجاح 100%، يغطي حساب التحلل لنظائر متنوعة، الحالات الحدّية، تحويل الوحدات، ومنطق التنبيهات (commit 44e9a55).
  - إضافة GitHub Actions workflow (.github/workflows/tests.yml) لتشغيل الاختبارات تلقائياً عند كل push أو pull request إلى main — تم التحقق من نجاح التشغيل الفعلي على GitHub.
  - تأكيد أن بطاقة "مؤشر التحلل الزمني" في لوحة التحكم تدعم فعلياً: قائمة منسدلة لكل المصادر المخزنة، منحنى تحلل مركب للمصادر متعددة النويدات (GetSourceCompositeDecayCurve)، ومحاور ديناميكية تُبنى حسب أطول عمر نصف بين النويدات. تم التحقق البصري من عمل هذه الميزة بنجاح.
