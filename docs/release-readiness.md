...
**حالة المستودع:** آخر commit مدفوع `7d71935` (الجولة 114) · CI `#100` أحمر (على جاني الوسيط `BorrowViewModel_ReceivesSourcesUpdatedMessage`) · 1075 اختباراً في CI (Release) / 1077 محلياً (Debug) · 5 تحذيرات بناء و0 أخطاء (تُملأ أرقام CI والتحذيرات بعد دفع الجولة 115)
...
**أمنية مؤجَّلة:** `PasswordHelper.VerifyPassword` يبتلع فشل BCrypt · `SeedData` يبتلع فشل ترقية هاش المدير
...
- `UsersViewModel.UpdateRoleSummaries` تكتب وصف صلاحيات الدور **نصاً ثابتاً** مُرمَّزاً في الكود بدل قراءته من `Role.Permissions` الفعلية، فقد يُعرض للمدير ما يخالف المخزَّن. كُشف في الجولة 111 وأُجّل لأن الجولة حظرت لمس أي ViewModel.
...
- **كنس الوسيط الشامل وإصلاح IsTestMode (مؤجّلان موثّقان):** جاني فشل CI `#100` كان اختبار `BorrowViewModel_ReceivesSourcesUpdatedMessage` عبر `WeakReferenceMessenger.Default` المشترك الذي أبلغ مستقبِلاً يحمل fixture ميتاً (`no such table: Sources`)، وعزلته الجولة 115 بحقن `IMessenger` في `BorrowViewModel` واختباره حصراً. كنس الوسيط الكامل في بقية شاشات المنظومة (سبعة ViewModels + تطبيق `IDisposable` في الخمسة التي تفتقر إليه + عزل وسائط الاختبارات القائمة) وإصلاح `DialogHelper.IsTestMode` لتفادي مشاركة حالة ساكنة بين فئات الاختبارات مؤجَّلان موثَّقان لما بعد النشر.
