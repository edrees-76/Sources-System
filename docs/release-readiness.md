...
**الجولة 213 (الإصدار v1.1.2 — قيد المراجعة):** الأساس `main` عند `dc8e5f2` (الجولة 212، PR #108 مدموجة). رفع الإصدار إلى 1.1.2 في `Sources.csproj` و`installer.iss`، وملاحظات الإصدار في `deploy/release-notes.md` (تتضمن بنود v1.1.1 لأنها لم تُنشر على GitHub). مهمة جديدة `publish-release` في `build-installer.yml` تعمل عند دفع وسم `v*` فقط: تتحقق من تطابق الوسم مع `<Version>`، وتنشر إصدار GitHub بالمثبِّت وملف SHA256 وقسم الإصدار من ملاحظات الإصدار، بصلاحية `contents: write` لتلك المهمة وحدها. **مطلوب:** دمج PR ثم دفع الوسم `v1.1.2` على commit الدمج، ثم مطابقة SHA256 وتجربة الترقية فوق v1.1.1 على جهاز حقيقي.

**الجولة 212 (رسوم لوحة التحكم الشريطية بعناصر WPF أصلية — مدموجة، PR #108، commit `dc8e5f2`):** الأساس `main` عند `7286184` (الجولة 211، PR #107 مدموجة). استبدال رسوم LiveCharts/Skia الثلاثة (نطاقات النشاط، النظائر، المواقع) بأشرطة أفقية WPF أصلية: المحور يبدأ من الصفر، العدد والنسبة ظاهران دائماً، لون واحد للأشرطة، «أخرى» سطر نصي بدل شريط، وتسميات النشاط بالوحدة (kBq…PBq) بدل الأُسّ، وإزالة `ArabicReshaper.ReshapeAndReverse` من هذه الرسوم لأن WPF تعالج العربية واتجاهها. النقر على أي صف يفتح نافذة كاملة بجدول مصادره (`DashboardSourcesWindow`) بدل اللوحة الجانبية. **ملاحظات إدريس البصرية (الدفعة الثانية):** توسيع صفوف سُلّم النشاط (كانت مضغوطة) مع LRM في «1 kBq»/«1 PBq»، واستبدال «مؤشر التحلل الزمني» (محور لوغاريتمي وتعبئات متراكبة) بمنحنيات «النشاط المتبقي» كنسبة من نشاط اليوم على محور خطي 0–100% لأعلى 5 مصادر، بمدى قابل للاختيار (1/5/10/30 سنة) ولوحة ألوان مُتحقَّق منها ووسيلة إيضاح بالنظير والنسبة؛ وضع المصدر المفرد بلا تغيير. لا تغيير في قاعدة البيانات أو الخدمات أو الصلاحيات. **مطلوب قبل الدمج:** قبول بصري من إدريس على بناء `bin\Debug\net8.0-windows` بالعربية والإنجليزية والوضعين الفاتح والداكن.

**الجولة 211 (تصحيحات إملائية في المثبِّت — مدموجة، PR #107، commit `7286184`):** الأساس `main` عند `23b566e` (الجولة 210، PR #106 مدموجة). تصحيح «بتثبي» ← «بتثبيت» وأربعة أخطاء همزة وصل في نصوص Inno Setup العربية الرسمية عبر `[Messages]` في `installer.iss`. لا تغيير في التطبيق ولا في رقم الإصدار.
