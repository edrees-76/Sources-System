...
**حالة المستودع:** `5a01303` مدفوع · CI `#90` أخضر · 1020 اختباراً محلياً / 995 في CI (قيد التحقق) · 5 تحذيرات بناء · 0 أخطاء
...
### ☑ ب1 — قيم `double` غير المنتهية في مدخلات المصادر النيترونية
`NaN` و `Infinity` تعبران الحارس `<= 0` لأن `NaN <= 0` تساوي `false`. تُحفظ في `NeutronSource.AnisotropyFactor` وتدخل الحسابات فيخرج رقم فاسد يُعرض ويدخل تقريراً.
النطاق: `SourcesViewModel.SaveAsync` + مسح بقية المدخلات الرقمية بالنمط نفسه.
يُغلق قبل ب4 حتى لا ترث الحقول الجديدة العيب. **أُنجز في الجولة 109.**

### ☐ ب2 — إنفاذ صلاحيات الأدوار في طبقة الخدمات
مُتحقَّق منه: `SourceService` لا يسأل عن الدور في أي دالة؛ `CurrentUser` يُستعمل لتسجيل الفاعل فقط. الحماية في الواجهة وحدها.
...
يسبقه: تأكيد أن `LocationService` و `UserService` و `BorrowService` على النمط نفسه.

### ☐ ب3 — جودة الاختبارات المكتوبة حديثاً
`ClipboardFeedbackTests` تقارن نصوصاً عربية حرفية · `BackupScanAndSettingsIntegrityTests:507` يمرّر تاريخاً مستقبلياً · اختبار الاقتطاع لا يفحص الاقتطاع. ومعها تصحيح الاقتطاع من 51 حرفاً إلى 50.
...
**أمنية مؤجَّلة:** القفل بعد المحاولات الفاشلة · `PasswordHelper.VerifyPassword` يبتلع فشل BCrypt · `SeedData` يبتلع فشل ترقية هاش المدير

**دين تقني:** `SettingsHelper` يهجر الملف القديم بعد فشل نسخ عابر (أُدخل في 105-ب) · `LoggerService.cs:9` يبني مسار السجلات يدوياً · 5 تحذيرات `CS8604` · تبعثر سلاسل الحالة في 45+ موضعاً بلا `enum` · تباين ألوان الحالة · ازدواج وحدات الحاسبة · تكرار `SimpleArabicStatus` · نقص `OldValues`/`NewValues` · `TestDataGeneratorService` بلا مصادر نيترونية · توحيد UTC · وضوح رسالة تكرار `SourceCode` عند الاسترجاع · مسح double.TryParse المتبقي (الجولة 109): 12 موضعاً متبقياً خارج SourcesViewModel — 6 مواضع تُحفظ في القاعدة (1 في LeakTestsViewModel لفحص التسرب، و5 في RadioisotopesViewModel للنظائر المشعة) و6 مواضع للعرض/حسابات تفاعلية في الذاكرة (5 في ActivityCalculatorViewModel، و1 في NeutronSourceTypesViewModel)
