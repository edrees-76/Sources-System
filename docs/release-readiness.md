...
- **التنفيذ:** كل نص استبدل بـ`TranslationHelper.GetString("Key") ?? "النص العربي الأصلي"` (ثابت) أو
  `GetFormat("Key", args...)` (بمتغيرات)، بنفس نمط الجولات 130-138 تماماً. 38 مفتاح ترجمة جديد
  (بعضها يُعاد استخدامه أكثر من مرة، مثل `TextEmptyNotSpecified`) أُضيف لكلا القاموسين. تواريخ العرض
