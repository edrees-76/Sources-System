...
**حالة المستودع:** الجولة 126 محلياً (ب4 — إضافة نشاط الأمريسيوم-241 الاختياري (قيمة + وحدة) على `NeutronSource` مع حساب اضمحلاله اعتماداً على `CalibrationDate`؛ لا واجهة في هذه الجولة) · 1124 اختباراً محلياً (Debug) / 1122 محلياً (Release) · 5 تحذيرات بناء (CS8604) و0 أخطاء
