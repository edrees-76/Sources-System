...
**حالة المستودع:** الجولة 126 محلياً (ب4 — إضافة نشاط الأمريسيوم-241 الاختياري (قيمة + وحدة) على `NeutronSource` مع حساب اضمحلاله اعتماداً على `CalibrationDate`؛ لا واجهة في هذه الجولة) · 1124 اختباراً محلياً (Debug) / 1122 محلياً (Release) · 3 تحذيرات بناء (CS8604) و0 أخطاء
...
3 تحذيرات بناء (CS8604 مسبقة، بلا علاقة بهذه الجولة) و0 أخطاء.
**يتبقى لإغلاق ب4 بالكامل:** وصل واجهة نشاط الأمريسيوم-241 وعرض النتيجة المحسوبة (جولة لاحقة)،
شاشة الأنواع المرجعية المستقلة الكاملة على نمط مكتبة النظائر (الحالية جزء من نافذة موجودة لا
شاشة مستقلة)، قسم المراجع العلمية — جولات لاحقة.
