...
**حالة المستودع:** آخر جولة مدموجة على `main` هي الجولة 198 (إصلاحات الحسابات
والصلاحيات + عزل الاختبارات: IsTestMode، IMessenger، commit `f2ba9aa`، PR #94).
الجولات 186–198 مدموجة جميعها فعلياً (إصلاح حلقة الإعداد المغلقة، جعل تغيير
...
وإصلاحات الحسابات والصلاحيات + عزل الاختبارات).
الجولة 199 (الفشل المغلق لطلب صلاحية مدير النظام + تحصين حالة التحرير العالقة
وحارس الشفاء الذاتي + عزل بيانات الاختبار عن `%LOCALAPPDATA%\Sources` الحقيقي)
قيد المراجعة (Draft PR، فرع `fix/round-199-fail-closed-and-editing-guard`).
تفاصيل كاملة في `docs\session-summary.md` §الجولة 199.
...
`enum` · تباين ألوان الحالة · ازدواج وحدات الحاسبة (انظر بند B في «قائمة ما بعد الإصدار» أعلاه) ·
تكرار `SimpleArabicStatus` · ~~`TestDataGeneratorService` بلا مصادر نيترونية~~ **أُغلق في الجولة 195
...
المستقبلية، خارج نطاق الجولة 197. **جرد D4 (الجولة 196):** مسح كل نوافذ Enter→حفظ
