...
المراجعة (Draft PR غير مدموج بعد، فرع `chore/round-195-small-items-batch`).
...
**متابعة CodeRabbit (فُرزت فعلياً في الجولة 195):** العدد الحقيقي **44** تعليقاً (لا 51 كما كان
مُقدَّراً سابقاً) عبر مراجعات PRs #18–#90 (المراجعة التلقائية لم تُفعَّل مطلقاً لهذه الـPRs). بعد
الفرز: **14** صفاً لا يزال صالحاً؛ **4** صفوف LOW نُفِّذت في الجولة 195 (H1 صيغة "غير مسجّل"/مسافة
الوحدة الفارغة، H2 `NeutronSourceType.DisplayName`، H3 تحميل واحد لكل تبديل تبويب، H4 تجميع
Regex + NBSP/NNBSP)؛ صفوف موجَّهة للجولة 196: `LocationFormWindow`/`UserFormWindow` وصفوف تقوية
الاختبارات؛ صفوف موجَّهة للجولة 197: منطق `NeutronSourceService.Restore` ومعالجة `AppDbContext`
القديمة لـAm-241/Be. ·
...
- **NeutronSourceType.DisplayName اللغوي (H2، مُنفَّذ جزئياً بالجولة 195):** أُضيف للنوع نفسه
  ورُبط في `SourcesView.xaml`/`NeutronSourceDetailsWindow.xaml`؛ صفوف CodeRabbit الأخرى
  الخاصة بـ`LocationFormWindow`/`UserFormWindow` وتقوية الاختبارات مُوجَّهة للجولة 196،
  ومنطق `NeutronSourceService.Restore`/معالجة `AppDbContext` القديمة لـAm-241/Be مُوجَّهة
  للجولة 197 (كلاهما محظور صراحة على الجولة 195).
