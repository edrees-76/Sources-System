...
في المحوِّلات القديمة؛ أصبحت Unknown (نص خام) لأنها ليست من القيم الأربع الفعلية.
...
6/6. صفر تحذيرات جديدة غير `CS8604` المعروفة (باستثناء 15 تحذيراً أسلوبياً `xUnit1026`
غير مؤثر في `StatusCatalogTests.cs`). لا ترحيل EF، لا تغيير مخطط أو قيم مخزَّنة. لم
تُلمس `LoginWindow`/`LoginView`/`SplashWindow`، `SystemResetService`، أو نص سجل
التدقيق. **CI:** قيد الانتظار بعد فتح الـPR.
