...
**حالة المستودع:** الجولة 161 قيد المراجعة (Draft PR غير مدموج): سد ثغرة صلاحيات التحرير في مسارات الإنشاء
والتعديل عبر ست خدمات (`AuthorizationGuard.RequireEditor` لم يكن يُستدعى إلا في الحذف/الاسترجاع)، مكتشَفة
بتدقيق خارجي مستقل ومؤكَّدة بفحص الكود مباشرة. **بند مفتوح يتطلب قرار القائد:** 42 حالة اختبار قديمة اعتمدت
ضمنياً على غياب الحارس؛ صُحِّحت تجهيزاتها بعد مراجعة القائد وموافقة إدريس (تفاصيل في §الجولة 161 أدناه) ·
1252/1252 نجاح محلياً (Debug)، صفر فشل، صفر تجاوز · تحذيرات بناء مسبقة بلا علاقة بهذه الجولة (CS8604 في
