...
- **96** — تصحيح `docs/schema-drift-report.md` وإعادة صياغته كسجل قرارات للمخطط الموحد على EF Core Migrations وإغلاق البنود بعد إثبات أسطر الكود. إغلاق الإسناد الكاذب في سجل الشهادات بـ `SourceDetailsViewModel` و `NeutronSourceDetailsViewModel` واستبداله بـ `"غير معروف"` مع `LoggerService.LogWarning`. استبدال `AddedBy` بـ `AddedByName` في `NeutronSourceDetailsViewModel` وتحديث ربط XAML. تنظيف استدعاء فحص الترحيلات في `RestoreBackup` لتفادي الأثر الجانبي في LocalAppData. إضافة واختبار مسار التراجع `Down()` وإصلاحه بتعطيل مؤقت للمفاتيح الخارجية مع `suppressTransaction: true` ليجتاز التحقق تحت الإنفاذ الافتراضي (قيد مستفاد: في SQLite لا يمكن ترتيب `migrationBuilder.Sql` نسبةً إلى إعادة بناء الجداول، وأي ترحيل مستقبلي يجمع بين الاثنين يجب أن يُفحص سكربته المولَّد قبل الاعتماد).

**آخر حالة:** 897 اختباراً محلياً ناجحاً (896 + 1 اختبار جديد).
...
### كبير
1. **توحيد `SourceCertificate.AttachedBy` إلى `Guid?` بمفتاح خارجي** (مؤجّل، جولة مخطط مستقلة).
2. **توسعة المصدر النيتروني:** فصل معدل الانبعاث إلى قيمة معايرة مرجعية وقيمة حالية محسوبة بالاضمحلال، ومراجعة معنى `AverageNeutronEnergyMeV` و `TypicalNeutronYield` (كلاهما فارغ في البذر حالياً)، وإضافة معامل تحويل التدفق إلى مكافئ الجرعة للأنواع التي لها معامل معياري فقط.
