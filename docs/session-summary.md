...
**آخر حالة:** `main` متزامن مع origin، 888 اختباراً محلياً / 886 في CI (فارق ثابت قدره 2 منذ الجولة 89، غير مقلق).
...
### كبير
1. **توحيد `AddedBy` إلى `Guid?`** — أكثر من 120 موضعاً في 20 ملفاً بينها شاشات XAML. حالياً `Source`/`Location`/`Radioisotope`/`BorrowRequest` تستخدم `string?` (اسم المستخدم) بينما الكيانات النيترونية تستخدم `Guid?`. القرار: التوحيد على `Guid?` لأن الاسم قابل للتغيير والحذف. **يُنصح بتقسيمها:** النموذج والخدمات في جولة، ثم ViewModels وXAML في جولة تالية.
2. **توسيع بيانات المصدر النيتروني** — 6 حقول على المصدر (الشركة المصنعة، رقم الموديل، النشاط GBq، تاريخ قياس النشاط، نوع الكبسولة، الأبعاد) + حقلان على النوع المرجعي (أقصى طاقة، نوع الطيف). **معطّل:** ملفا `P074_scr` و`TRS393` غير موجودين في `Resources/References/`.
...
7. توثيق تناقض `AddedBy` والمفتاح الميت `MsgErrSourceCodeDeleted` في `docs/schema-drift-report.md`.
8. إصلاح Bidi في 3 رسائل حوار: `LeakTestsViewModel`، `BorrowViewModel`، `SettingsViewModel`.
9. ترجمة تبويب "بيانات تجريبية (Debug)" وشارة "مدير النظام" في القائمة السفلية.

### ملاحظات مفتوحة
- فحص التوافق في `RestoreBackup` يستخدم `LIKE '%InitialSchema%'` — يحتاج تشديداً حين يصير عدد الترحيلات أكثر من واحد.
