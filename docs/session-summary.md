...
- الواجهة تُخفي لتحسين التجربة، والخدمة تمنع لحماية البيانات. إخفاء زر ليس حماية: شاشة واحدة تنسى الإخفاء تفتح الثغرة. الحارس في `Services/AuthorizationGuard` يُستدعى في بداية كل عملية مدمّرة أو إدارية قبل أي كتابة. ولا يُنشأ نظام صلاحيات موازٍ — الآليات المعتمدة هي `User.IsAdmin` و `User.IsEditor` و `User.HasSectionPermission(section)` وحدها.
